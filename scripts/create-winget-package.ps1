...
# Add manifest file
$version = if ($releaseVersion) { $releaseVersion -replace '(\d+\.\d+\.\d+)(-.+)?', '$1.0$2' } else { '0.0.1.0' }
(Get-Content (Resolve-Path .\scripts\winget\AppxManifest.xml)).Replace('0.0.1.0', $version) | Set-Content (Join-Path $buildDir AppxManifest.xml)
